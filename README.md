...
当前版本 **0.9.2**：新增实物部件老化、条件剩余寿命、默认修复如新及可选最小修复；年龄随备件流转，检查计时与部件年龄独立。提供中文年龄结果与全轮 CSV。见[0.9.2使用说明](docs/V0.9.2.md)；安装交付状态见[验证记录](docs/VERIFICATION.md)。

本轮 M2 范围已实现并通过验收；多工序准备另行设计，见[下一步计划](docs/NEXT_VERSION_PLAN.md)。完整顺序见 [开发路线图](docs/ROADMAP.md)；当前案例与Git复现说明见 [案例目录](docs/cases/README.md)。
...
- 另有7张SimLab独立扩展表（36字段），包含日历维修、飞行小时检查及逐架初始小时；不混入原厂字典，不宣称原厂兼容。
- 建模界面展示29张支持表、235字段，保留01–07七步业务顺序。检查及初始小时归04维修与保障作业；尚未支持的数据仍在运行前拦截。
