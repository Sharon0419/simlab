...
当前版本 **0.11.0**：新增外部采购与部件寿命报废。按临界库存或周期补足目标库存，固定交期，采购与调运优先较早到货并可拆分；支持各层级部件累计小时和修复次数到限退出。新增中文采购、报废记录和实物寿命台账。界面保持说明按需展开。见[0.11使用说明](docs/V0.11.md)与[验证记录](docs/VERIFICATION.md)。

M3本轮范围为三级供应、维修方式、部件预防维修、采购与寿命报废；供应中断、横向调拨与多工序准备仍为后续项。完整顺序见[开发路线图](docs/ROADMAP.md)。
...
根目录只保留 `启动SimLab.cmd` 一个启动入口，双击后在“项目库”搜索并切换案例；软件内点击左侧“08 项目库”即可换项目。案例数据和已保存结果独立保留，案例工具不再生成单独的cmd。本机本次并排安装位于 `dist/SimLab-v0.11.0/SimLab.exe`，统一入口优先启动新版；旧 `dist/SimLab` 保留，避免替换正在运行的旧窗口。便携压缩包中的入口仍为 `SimLab/SimLab.exe`。便携版需要保留整个 `SimLab` 文件夹中的 `_internal` 目录，不要只复制 exe。运行不需要服务器、账号或互联网。
...
- 另有18张SimLab独立扩展表（94字段），包含多级供应、维修方式、部件预防维修、老化、日历维修和飞行小时检查；不混入原厂字典，不宣称原厂兼容。
- 建模界面展示40张支持表、293字段，保留01–07七步业务顺序。检查及初始小时归04维修与保障作业；尚未支持的数据仍在运行前拦截。
