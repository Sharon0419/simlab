...
当前版本 **0.12.0**：新增装备两层 n 中取 k，以及维修、出动准备、日历检查、飞行小时检查的自定义多工序，支持紧前紧后、并行汇合和每道工序独立资源。概览可新建“冗余与多工序示例”，结果新增“保障多工序”及全部轮次导出。界面保持说明按需展开。见[0.12使用说明](docs/V0.12.md)与[验证记录](docs/VERIFICATION.md)。

已有三级供应、维修方式、部件预防维修、采购与寿命报废，本轮增加冗余与保障工序网络；供应中断、横向调拨及运输容量仍为后续项。完整顺序见[开发路线图](docs/ROADMAP.md)。
...
根目录只保留 `启动SimLab.cmd` 一个启动入口，双击后在“项目库”搜索并切换案例；软件内点击左侧“08 项目库”即可换项目。案例数据和已保存结果独立保留，案例工具不再生成单独的cmd。本机本次并排安装位于 `dist/SimLab-v0.12.0/SimLab.exe`，统一入口优先启动新版；旧 `dist/SimLab` 保留，避免替换正在运行的旧窗口。便携压缩包中的入口仍为 `SimLab/SimLab.exe`。便携版需要保留整个 `SimLab` 文件夹中的 `_internal` 目录，不要只复制 exe。运行不需要服务器、账号或互联网。
...
- 另有22张SimLab独立扩展表（115字段），包含冗余、保障工序网络、多级供应、维修方式、部件预防维修、老化、日历维修和飞行小时检查；不混入原厂字典，不宣称原厂兼容。
- 建模界面展示44张支持表、314字段，新增“08 保障多工序”分组；尚未支持的数据仍在运行前拦截。
...
采购与报废体验：在项目概览点击“新建采购与报废示例”，修改采购策略和部件寿命限值后运行。现有项目库仍保留此前两个三级供应案例；新示例由用户按需创建。
