...
当前版本 **0.10.0**：新增三级供应、临界库存或周期调运、原位/换件混合维修及部件预防维修。提供中文供应与维修明细、全轮 CSV 和独立示例。见[0.10使用说明](docs/V0.10.md)；安装交付状态见[验证记录](docs/VERIFICATION.md)。
...
根目录只保留 `启动SimLab.cmd` 一个启动入口，双击后在“项目库”搜索并切换案例；软件内点击左侧“08 项目库”即可换项目。案例数据和已保存结果独立保留，案例工具不再生成单独的cmd。本机本次并排安装位于 `dist/SimLab-v0.10.0/SimLab.exe`，统一入口优先启动新版；旧 `dist/SimLab` 保留，避免替换正在运行的旧窗口。便携压缩包中的入口仍为 `SimLab/SimLab.exe`。便携版需要保留整个 `SimLab` 文件夹中的 `_internal` 目录，不要只复制 exe。运行不需要服务器、账号或互联网。
