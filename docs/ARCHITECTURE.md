...
0.4：固定值守沿用 MissionManager 事件积分和设备分配，增加带截止时间的补位预留与最低保障统计；维修引擎通过设备状态继续与任务联动。独立 SimLabDutyRule 输入由 operations.py 编译，duty.py 负责计划追加与共享剖面隔离，ui/duty.py 提供中文编辑和预览。项目扩展版本为2，读取旧版本1，保存时升级。完整口径见 [V0.4.md](V0.4.md)。
