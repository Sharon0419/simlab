...
当前版本 **0.10.1**：工作页面默认显示操作和数据，字段/计算说明按需展开，解释移至悬浮提示；保留错误、单位和结果范围标识。0.10功能包括：新增三级供应、临界库存或周期调运、原位/换件混合维修及部件预防维修。提供中文供应与维修明细、全轮 CSV 和独立示例。见[0.10使用说明](docs/V0.10.md)；安装交付状态见[验证记录](docs/VERIFICATION.md)。

本轮为已批准的 M3 供应与维修方式；多工序准备、采购报废与供应中断另行设计，见[下一步计划](docs/NEXT_VERSION_PLAN.md)。完整顺序见 [开发路线图](docs/ROADMAP.md)；当前案例与Git复现说明见 [案例目录](docs/cases/README.md)。
...
根目录只保留 `启动SimLab.cmd` 一个启动入口，双击后在“项目库”搜索并切换案例；软件内点击左侧“08 项目库”即可换项目。案例数据和已保存结果独立保留，案例工具不再生成单独的cmd。本机本次并排安装位于 `dist/SimLab-v0.10.1/SimLab.exe`，统一入口优先启动新版；旧 `dist/SimLab` 保留，避免替换正在运行的旧窗口。便携压缩包中的入口仍为 `SimLab/SimLab.exe`。便携版需要保留整个 `SimLab` 文件夹中的 `_internal` 目录，不要只复制 exe。运行不需要服务器、账号或互联网。
...
- 另有16张SimLab独立扩展表（82字段），包含多级供应、维修方式、部件预防维修、老化、日历维修和飞行小时检查；不混入原厂字典，不宣称原厂兼容。
- 建模界面展示38张支持表、281字段，保留01–07七步业务顺序。检查及初始小时归04维修与保障作业；尚未支持的数据仍在运行前拦截。
- 建模表名、列标题、字段说明、单位、约束与固定选项以中文显示；搜索支持中文字段名称和原始代码。原始表/字段代码可通过悬浮提示查看，项目编号与关联数据保持原值，CSV继续使用原始字段代码。
...
飞机主案例：进入“08 项目库”，搜索“飞机再次出动保障”，打开“飞机再次出动保障 · 2组 · 12架 · 1000次”。1组、4组是保障资源对比方案。“飞机成功点 · 12架备用池 · 1000次”是此前不计准备资源容量的版本。车辆项目是自带入门示例。
...
M3模式的三级网络、库存位置、策略触发和维修分流见[0.10说明](docs/V0.10.md)；以下运输及一件换一件补货描述适用于旧模式。

旧模式支持 System→LRU→SRU 串联结构、多个系统与部署地点、连续或固定任务需求模式、两级保障、有限备件、直接修复与内部换件、检测测试、运输及组合资源排队。层级语义见 0.3 说明。
...
- 初始化在库数量取 `ISTOH`，未填写则取 `STSIZ`。旧模式采用闭环一件换一件补充；库存不足时等待修复件或上级补充。
...
本版尚未实现：超过LRU/SRU两层的部件维修、PRU、冗余/RBD、非关键故障、日历老化、复杂任务调度和完整原厂任务成功语义、完整原厂预防维修算法、采购报废、多级/横向供应、时变扰动、自动优化及多人修改自动合并。日历维修和飞行小时检查仅用于固定飞行模式。
