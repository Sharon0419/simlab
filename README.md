...
当前版本 **0.4.0**：固定时段值守、最低保障判定、优先级分配、补位准备、按日计划预览和不达标时段分析。保留 LRU/SRU 多层维修与任务班次。见 [0.4 使用及计算口径](docs/V0.4.md)。

本次实施依据 [0.4 固定值守契约](docs/V0.4-SPEC.md)；计划性维修仍待下一阶段。
...
- 另有 2 张 SimLab 独立扩展表（11 字段）定义检测测试和值守规则；不混入原厂字典，不宣称原厂兼容。
