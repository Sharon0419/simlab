...
当前版本 **0.3.0**：基地整件换 LRU、修理站内部 SRU 维修、检测测试、组成树、维修工序统计及实物部件追溯。保留 0.2 任务日历与班次。更新说明见 [0.3 使用及计算口径](docs/V0.3.md)。

本次实施依据 [0.3 多层维修计划](docs/NEXT_VERSION_PLAN.md)；计划性维修仍待下一阶段。
...
体验 0.3：进入“项目概览 → 新建多层维修示例”，查看“组成结构”；在“模型数据 → SimLabDepotProcess”修改检测/测试时间，在“仿真实验”运行，再查看“维修统计 / 工序耗时 / 部件实例”。
...
- 另有 1 张 SimLab 独立扩展表（6 字段）定义检测和修后测试；不混入原厂字典，不宣称原厂兼容。
...
自动化桌面检查覆盖连续、任务、多层维修三个场景，包含字段编辑、后台计算、取消、结果持久化、交换与 CSV，产出八张界面截图及 smoke-result.json。执行正常桌面启动前移除 QT_QPA_PLATFORM 环境覆盖。
