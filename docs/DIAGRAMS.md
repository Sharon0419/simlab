...
0.4：值守计划页 → 原任务表＋SimLabDutyRule → 编译校验 → MissionManager（预留、补位、任务供给）↔ 设备故障／LRU换修／SRU维修 → 事件积分 → 达标率、合格率、首轮不达标区间 → SQLite结果快照与任务CSV。原固定起止时间贯穿整个流程，补位不顺延结束时间。

0.3：后台编译层增加两层组成校验，仿真层增加部件实例和 SRU 维修工序；result.json 包含 maintenance/components，界面增加组成树和维修明细。完整口径见 [V0.3.md](V0.3.md)。

0.2 扩展：编译层调用 operations.py 处理固定需求窗口和资源班次；引擎调用 missions.py 分配设备并积分供需缺口；结果 JSON 新增 mission，界面增加供需曲线、任务窗口和缺口表。下图的进程与持久化架构不变；末尾原“任务调度尚未实现”指 0.1 基线，0.2 边界见 [V0.2.md](V0.2.md)。
